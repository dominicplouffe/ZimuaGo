...
func TestSiblings(t *testing.T) {
	...
	siblings := make([]MoveScore, 5)
	res := zg.alphaBetaNM(game.Position(), 5, -9999999999, 9999999999, 1, false, false, siblings)

	prevs := make([]string, 5)
	if res.move.String() == "a1a1" {
		t.Error("Something terrible happened")
	}

	for i, s := range siblings {
		mv := s.move.String()
		if stringInSlice(mv, prevs) {
			t.Error(mv + " already in siblings")
		}
		prevs[i] = mv
	}
}
...
func TestNegaMax(t *testing.T) {
	...
	if res.move.String() != "e2e4" || res.score != 55 {
		t.Error("Depth 1 NM is incorrect: ", res.move)
	}
